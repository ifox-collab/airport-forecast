...
function windRose(runways, dir, kt, compact) {
  ...
  const arrow = (numDir == null || !Number.isFinite(numDir)) ? "" : '<g transform="rotate('+numDir+' '+cx+' '+cy+')"><path d="M '+cx+' '+(cy-48)+' L '+(cx-14)+' '+(cy-72)+' L '+(cx-5)+' '+(cy-72)+' L '+(cx-5)+' '+(cy-96)+' L '+(cx+5)+' '+(cy-96)+' L '+(cx+5)+' '+(cy-72)+' L '+(cx+14)+' '+(cy-72)+' Z" fill="#4ade80" stroke="#166534" /></g>';
