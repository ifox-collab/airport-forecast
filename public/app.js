...
function windRose(runways, dir, kt, compact) {
  ...
  const kts = Number(kt);
  const band = !Number.isFinite(kts) ? 0 : kts >= 35 ? 3 : kts >= 25 ? 2 : kts >= 10 ? 1 : 0;
  const dartFill = band === 3 ? "#ef4444" : band === 2 ? "#f59e0b" : "#4ade80";
  const dartStroke = band === 3 ? "#f59e0b" : band === 2 ? "#b45309" : "#166534";
  const dartDur = band === 3 ? "1.35s" : band === 2 ? "1.75s" : band === 1 ? "2.6s" : "3.6s";
  const arrow = (numDir == null || !Number.isFinite(numDir)) ? "" : (
    ...
    + '<g class="wind-arrow">'
    + '<animateTransform attributeName="transform" type="translate" from="0 -46" to="0 130" dur="'+dartDur+'" repeatCount="indefinite"/>'
    + '<animate attributeName="opacity" values="0;1;1;0" keyTimes="0;0.18;0.78;1" dur="'+dartDur+'" repeatCount="indefinite"/>'
    + '<path d="M '+cx+' '+(cy-8)+' L '+(cx-11)+' '+(cy-30)+' L '+(cx-4)+' '+(cy-30)+' L '+(cx-4)+' '+(cy-52)+' L '+(cx+4)+' '+(cy-52)+' L '+(cx+4)+' '+(cy-30)+' L '+(cx+11)+' '+(cy-30)+' Z" fill="'+dartFill+'" stroke="'+dartStroke+'"/>'
    + '</g></g>'
  );
...
function parseRawMetar(text) {
  ...
  return {
    raw, windDir: windM ? (windM[1] === "VRB" ? "VRB" : Number(windM[1])) : null,
    windKt: windM ? Number(windM[2]) : null, gustKt: windM && windM[3] ? Number(windM[3]) : null,
    visM: visTok ? Number(visTok) : null, clouds, ceilingFt: ceilingFeet(clouds),
...
function paint() {
  ...
  const t = new Date();
  document.getElementById("updated").textContent = "Updated "+String(t.getUTCHours()).padStart(2,"0")+":"+String(t.getUTCMinutes()).padStart(2,"0")+" UTC";
  tickClocks();
}
async function load() {
...
function schedule() { setInterval(load, 5*60*1000); setInterval(tickClocks, 1000); setInterval(function(){ location.reload(); }, 3*60*60*1000); }
(function scroll() {
